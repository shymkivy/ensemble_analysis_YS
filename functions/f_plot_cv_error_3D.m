...
figure; hold on;
if numel_x == 1
    plot(squeeze(grid_y(:,:,1)), mean(grid_z,3));
    if numel_rep > 1
        errorbar(squeeze(grid_y(:,:,1)), mean(grid_z,3), std(grid_z,[],3))
    end
    xlabel(y_var, 'interpreter', 'none');
    ylabel(z_var, 'interpreter', 'none');
elseif numel_y == 1
    plot(squeeze(grid_x(:,:,1)), mean(grid_z,3));
    if numel_rep > 1
        errorbar(squeeze(grid_x(:,:,1)), mean(grid_z,3), std(grid_z,[],3))
    end
